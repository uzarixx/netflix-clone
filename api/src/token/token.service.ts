...
export class TokenService {
  ...
  constructor(@InjectModel(Tokens) private tokensRepository: typeof Tokens, private mailService: MailService, private accountsService: AccountsService) {
  }


  async createAuthToken(accounts: Accounts) {
    const tokenGen = uuid.v4();
    const expiresAt = Date.now() + 100 * 60 * 5;
